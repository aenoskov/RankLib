...
void indri::infnet::OrderedWindowNode::prepare( int documentID ) {
  ...
  for( ; _pointers[0].iter != _pointers[0].end; (_pointers[0].iter)++ ) {
    ...
    if( match ) {
      ...
      // last term
      _extents.push_back( indri::index::Extent( _pointers.front().iter->begin, _pointers.back().iter->end ) );
    }
  }
}
...
void indri::infnet::OrderedWindowNode::annotate( indri::infnet::Annotator& annotator, int documentID, int begin, int end ) {
