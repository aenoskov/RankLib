...
void indri::infnet::InferenceNetwork::_buildComplexEvaluatorList() {
  std::vector<indri::infnet::EvaluatorNode*>::iterator iter;
  _complexEvaluators.clear();
  
  for( iter = _evaluators.begin(); iter != _evaluators.end(); iter++ ) {
    if( (*iter)->isComplex() ) {
      _complexEvaluators.push_back( *iter );
    }
  }
}
...
void indri::infnet::InferenceNetwork::_evaluateIndex( indri::index::Index& index ) {
  ...
  std::cout << "Scored: " << scoredDocuments << std::endl;
