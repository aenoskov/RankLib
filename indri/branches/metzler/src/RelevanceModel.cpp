...
void indri::query::RelevanceModel::_scoreGrams() {
  ...
  for( iter = _gramTable.begin(); iter != _gramTable.end(); iter++ ) {
	...
	gramDocCount = _environment.documentCount( gram->terms[0] );
      } else {
	...
	indri::api::QueryEnvironment::ExpressionStatistics* gramStats = _environment.expressionStatistics( s.str() );
	gramCount = gramStats->occurrences;
	gramDocCount = gramStats->documentOccurrences;
	delete gramStats;
      }

      double gramFrequency = gramCount / collectionCount;
      function = indri::query::TermScoreFunctionFactory::get( _smoothing, gramCount, collectionCount, gramDocCount, docCount );
    }
