...
public class CollectionFrequencyComparator implements Comparator<String> {

  Map<String, TermFrequency> map = new HashMap<>();

  public CollectionFrequencyComparator(Map<String, TermFrequency> map) {
    this.map.putAll(map);
  }

  @Override
  public int compare(String s1, String s2) {
    if (map.get(s1).getCollectionTermFrequency() >= map.get(s2).getCollectionTermFrequency()) {
      return -1;
    } else {
      return 1;
    }
